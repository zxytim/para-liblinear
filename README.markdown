What is Para-LIBLINEAR
======================
LIBLINEAR(http://www.csie.ntu.edu.tw/~cjlin/liblinear/),
is famous for its training speed on large scale data. However, when
training classification problems with a more than one class (supposely n classes),
LIBLINEAR adopts one-vs-all strategy and train all n classifiers sequentially,
which greatly encumbered the training speed.

As the training process of each classifier does not depend on each other,
we can just train those n classifiers concurrently.

Here, I simply adopted ThreadPool(https://github.com/progschj/ThreadPool) to
do the trick for me, but a decent c++ compiler with c++11 support would be
required.
